...
bool HSCPSelector::PassHSCPpresel_test(int i){
   if (i<0 || i>(int)Pt.GetSize()) return false;
   return Pt[i]>50 && abs(eta[i])<2.1;
}
...
selections_.push_back(&HSCPSelector::PassHSCPpresel_test);
selLabels_.push_back("test");
